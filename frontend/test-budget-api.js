...
async function testBudgetAPI() {
  ...
  try {
    ...
    const normalizedBudgets = budgets.map(budget => {
      const amount = budget.amount && typeof budget.amount === 'object' ? 
        (budget.amount.raw ?? 0) : (budget.amount ?? 0);
      const spent = budget.spent_amount && typeof budget.spent_amount === 'object' ? 
        (budget.spent_amount.raw ?? 0) : (budget.spent_amount ?? 0);
      const remaining = budget.remaining_amount && typeof budget.remaining_amount === 'object' ? 
        (budget.remaining_amount.raw ?? 0) : (budget.remaining_amount ?? (amount - spent));
