    ...
    <div class="summary-card categories" @click="showDetails('categories')">
      ...
      <div class="card-content">
        ...
        <div class="card-body">
          ...
          <div class="card-value">
            <AnimatedCounter v-if="!loading" :value="totalCategories" :duration="1500" />
            <div v-else class="loading-skeleton"></div>
          </div>
...
<script setup>
  ...
  const totalExpenses = computed(() => {
    return expensesStore.expenses.reduce((total, expense) => total + parseFloat(expense.amount || 0), 0).toFixed(2)
  })
  ...
  const averageMonthly = computed(() => {
    ...
    const months = new Set(expensesStore.expenses.map(expense => expense.date.substring(0, 7)))
