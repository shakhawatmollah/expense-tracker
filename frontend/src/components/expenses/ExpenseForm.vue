      ...
      <div class="modal-body">
        <form @submit.prevent="submitForm" class="expense-form">
          ...
          <div class="form-actions">
            <Button variant="outline" @click="closeModal" :disabled="loading">Cancel</Button>
            <button
              type="submit"
              :disabled="loading"
              :class="['colorful-budget-btn', isEditing ? 'update-expense-btn' : 'create-expense-btn']"
            >
  ...
  watch(
    ...
    newExpense => {
      if (newExpense) {
        ...
        form.category_id = typeof newExpense.category === 'object' ? newExpense.category.id : newExpense.category_id
      } else {
        resetForm()
      }
    },
...
<style scoped>
  ...
  .create-expense-btn {
    background: linear-gradient(45deg, #22c55e 0%, #10b981 25%, #06b6d4 50%, #3b82f6 75%, #6366f1 100%);
  ...
  .update-expense-btn {
    background: linear-gradient(45deg, #f97316 0%, #ef4444 25%, #ec4899 50%, #a855f7 75%, #8b5cf6 100%);
    background-size: 300% 300%;
