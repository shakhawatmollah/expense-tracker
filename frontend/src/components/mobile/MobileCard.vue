    ...
    >
      ...
      <div v-if="hasHeader" class="card-header">
        ...
        <div class="header-right">
          ...
          <button v-if="showMenuButton" @click="toggleMenu" class="menu-button" :class="{ active: menuOpen }">
      ...
      <!-- Touch Ripple Effect -->
      <div v-if="showRipple" class="touch-ripple" :style="rippleStyles"></div>
    </div>
  </TouchGestureWrapper>
</template>
...
<script>
  ...
  export default {
    ...
    emits: ['click', 'double-click', 'long-press', 'swipe-action', 'menu-action', 'selection-change'],
    setup(props, { emit }) {
      // Reactive state
      ...
      // Computed properties
      const hasHeader = computed(() => props.title || props.subtitle || props.icon || props.badge)

      const hasFooter = computed(
        () => props.timestamp || props.tags?.length || props.amount !== null || props.progress > 0
      )

      const showMenuButton = computed(() => props.contextMenuItems.length > 0)

      const showProgress = computed(() => props.progress > 0 && props.progress <= 100)

      const leftActions = computed(() => props.swipeActions.filter(action => action.position === 'left'))
