  ...
  <div class="empty-state" :class="[variant, size]">
    <div class="empty-container">
      ...
      <div class="empty-actions" v-if="showActions">
        <button v-if="showPrimary" @click="$emit('primary-action')" class="action-btn primary" :disabled="loading">
          <i :class="primaryIcon" v-if="primaryIcon"></i>
          {{ loading ? loadingText : primaryText || defaultPrimaryText }}
        </button>

        <button v-if="showSecondary" @click="$emit('secondary-action')" class="action-btn secondary">
          <i :class="secondaryIcon" v-if="secondaryIcon"></i>
          {{ secondaryText || defaultSecondaryText }}
        </button>

        <router-link v-if="showLink && linkTo" :to="linkTo" class="action-btn link">
...
<script setup>
  ...
  const props = defineProps({
    type: {
      ...
      default: 'general',
      validator: value =>
        ['general', 'expenses', 'categories', 'budgets', 'analytics', 'search', 'filter', 'upload'].includes(value)
    },
    variant: {
      type: String,
      default: 'default',
