        ...
        <div class="settings-content">
          <div class="setting-item">
            <label class="setting-label">
              <input type="checkbox" v-model="isAutoRefreshEnabled" @change="handleAutoRefreshToggle" />
          ...
          <div class="setting-item">
            <label class="setting-label">
              <input type="checkbox" v-model="syncOnFocus" @change="handleSyncOnFocusToggle" />
          ...
          <div class="setting-item">
            <label class="setting-label">
              <input type="checkbox" v-model="showNotifications" @change="handleNotificationToggle" />
  ...
  export default {
    ...
    setup(props, { emit }) {
      ...
      const performRefresh = async (isManual = false) => {
        // Prevent concurrent refresh calls (critical for recursion prevention)
        if (isSyncing.value) {
          return
        }
        ...
        try {
          ...
          // Perform the actual refresh with timeout protection
          ...
          // Show error notification only for non-timeout errors or manual refresh
          if (showNotifications.value && (isManual || error.message !== 'Refresh timeout')) {
            window.notify?.error('Sync Failed', 'Failed to refresh data. Will retry automatically.')
          }

          emit('refresh-error', { error, isManual })

          // Retry logic for auto-refresh only
      ...
      const scheduleRetry = () => {
        ...
        retryTimer = setTimeout(() => {
          retryTimer = null // Clear the timer reference
